...
- Identificadores contractuales antes sic, **ya corregidos** en la pila completa (entidad, DTOs, mapper, servicios, tests, frontend): `minesAroundQuantity`/`mines_around_quantity` (desde `Arround…Quantiy`) y `isRevealed`/`is_revealed` (desde `isRotated`/`id_rotated` — el campo siempre significó "revealed"). En BD vía migración de rename `v0.0.3` (la DDL `v0.0.1` conserva los nombres originales). Ver apéndice Fase 6.
...
- **Corrección de identificadores cross-cutting** (entidad, DTOs, mapper, servicios, tests, contract del frontend):
  - `minesArroundQuantiy`/`mines_arround_quantity` → `minesAroundQuantity`/`mines_around_quantity`.
  - `isRotated`/`id_rotated` → `isRevealed`/`is_revealed` (el campo siempre significó "revealed"; prefijo `id_`→`is_` para coincidir con `is_flagged`).
  - **DB**: la migración `v0.0.1` (create-table-boxes) se deja con los nombres **originales** (historia fiel) y se añadió la migración **`v0.0.3/DDL/1773100000000-rename-box-columns`** que hace `ALTER TABLE "boxes" RENAME COLUMN` para ambas columnas (`up` renombra, `down` revierte). Reaplica los nombres ya existentes en BDs dev migradas con la v0.0.1 antigua y también funciona en setups frescos (crea con nombres viejos → renombra).
